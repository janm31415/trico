...
namespace trico
  ...
  struct old_archive
    {
    old_archive() : version(0), next_stream_type(empty) {}
    FILE* f;
    uint32_t version;
  ...
  namespace
    ...
    void write_header(old_archive* arch)
      {
    ...
    void read_next_stream_type(old_archive* arch)
      {
      if (!feof(arch->f))
        {
    ...
    bool read_header(old_archive* arch)
      {
      uint32_t Trco;
  ...
  void* open_old_archive_for_writing(const char* filename)
    {
    old_archive* arch = new old_archive();
  ...
  void* open_old_archive_for_reading(const char* filename)
    {
    old_archive* arch = new old_archive();
  ...
  void close_old_archive(void* a)
    {
    old_archive* arch = (old_archive*)a;
    if (arch->f)
      fclose(arch->f);
    delete arch;
    }

  uint32_t get_version_old(void* a)
    {
    old_archive* arch = (old_archive*)a;
    return arch->version;
    }

  void write_vertices_old(void* a, uint32_t nr_of_vertices, const float* vertices)
    {
    old_archive* arch = (old_archive*)a;
  ...
  void write_vertices_old(void* a, uint32_t nr_of_vertices, const double* vertices)
    {
    old_archive* arch = (old_archive*)a;
  ...
  void write_triangles_old(void* a, uint32_t nr_of_triangles, const uint32_t* tria_indices)
    {
    old_archive* arch = (old_archive*)a;
  ...
  void write_triangles_old(void* a, uint32_t nr_of_triangles, const uint64_t* tria_indices)
    {
    old_archive* arch = (old_archive*)a;
  ...
  e_stream_type get_next_stream_type_old(void* a)
    {
    old_archive* arch = (old_archive*)a;
    return arch->next_stream_type;
    }

  bool read_vertices_old(void* a, uint32_t* number_of_vertices, float** vertices)
    {
    old_archive* arch = (old_archive*)a;
    *number_of_vertices = 0;
    *vertices = nullptr;
    if (get_next_stream_type_old(arch) != vertex_float_stream)
      return false;
  ...
  bool read_vertices_old(void* a, uint32_t* number_of_vertices, double** vertices)
    {
    old_archive* arch = (old_archive*)a;
    *number_of_vertices = 0;
    *vertices = nullptr;
    if (get_next_stream_type_old(arch) != vertex_double_stream)
      return false;
  ...
  bool read_triangles_old(void* a, uint32_t* nr_of_triangles, uint32_t** tria_indices)
    {
    old_archive* arch = (old_archive*)a;
    *nr_of_triangles = 0;
    *tria_indices = nullptr;
    if (get_next_stream_type_old(arch) != triangle_uint32_stream)
      return false;
  ...
  bool read_triangles_old(void* a, uint32_t* nr_of_triangles, uint64_t** tria_indices)
    {
    old_archive* arch = (old_archive*)a;
    *nr_of_triangles = 0;
    *tria_indices = nullptr;
    if (get_next_stream_type_old(arch) != triangle_uint64_stream)
      return false;
