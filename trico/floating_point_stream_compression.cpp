...
namespace trico
  ...
  */
  ...
  void compress(uint32_t* nr_of_compressed_bytes, uint8_t** out, const float* input, const uint32_t number_of_floats, uint32_t hash1_size_exponent, uint32_t hash2_size_exponent)
    {
    ...
    *p_out++ = (uint8_t)((number_of_floats) & 0xff);
    ...
    for (uint32_t i = 0; i < number_of_floats; ++i)
      ...
      hash1 = compute_hash1(hash1, value, hash1_size_exponent, hash1_mask);
      ...
      hash2 = compute_hash2(hash2, stride, hash2_size_exponent, hash2_mask);
  ...
  void compress(uint32_t* nr_of_compressed_bytes, uint8_t** out, const double* input, const uint32_t number_of_doubles, uint64_t hash1_size_exponent, uint64_t hash2_size_exponent)
    {
    hash1_size_exponent = (hash1_size_exponent >> 1) << 1;
    hash2_size_exponent = (hash2_size_exponent >> 1) << 1;
    if (hash1_size_exponent > 30)
  ...
  void decompress(uint32_t* number_of_doubles, double** out, const uint8_t* compressed)
    {
    uint8_t hash_info = *compressed++;
