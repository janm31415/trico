...
namespace trico
  ...
  TRICO_API void transpose_uv_aos_to_soa(float** u, float** v, const float* uv, uint32_t nr_of_uv_positions);

  TRICO_API void transpose_uv_soa_to_aos(float** uv, const float* u, const float* v, uint32_t nr_of_uv_positions);

  TRICO_API void transpose_uv_aos_to_soa(double** u, double** v, const double* uv, uint32_t nr_of_uv_positions);

  TRICO_API void transpose_uv_soa_to_aos(double** uv, const double* u, const double* v, uint32_t nr_of_uv_positions);

  TRICO_API void transpose_uint16_aos_to_soa(uint8_t** b1, uint8_t** b2, const uint16_t* indices, uint32_t nr_of_indices);

  TRICO_API void transpose_uint16_soa_to_aos(uint16_t** indices, const uint8_t* b1, const uint8_t* b2, uint32_t nr_of_indices);
